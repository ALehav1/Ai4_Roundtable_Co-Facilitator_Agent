...
const RoundtableCanvasV2: React.FC = () => {
  ...
  const getPhaseSpecificContext = useCallback((
    ...
    switch(analysisType) {
      ...
      case 'synthesis':
        return {
          instruction: `Synthesize the discussion about "${currentQuestion?.title}". Focus on connecting participant responses to the objective: ${guidance?.objective || 'session goals'}`,
          frameworkToApply: guidance?.framework ? `Apply the ${guidance.framework.title} framework` : null
        };
        
      case 'followup':
        return {
          instruction: `Generate follow-up questions based on gaps in the "${currentQuestion?.title}" discussion. Consider unused prompts from the guide.`,
      ...
      case 'executive':
        return {
          instruction: `Create an executive summary of the session, focusing on Phase ${phase + 1}: ${currentQuestion?.title}`,
          keyMessages: guidance?.keyMessage ? [guidance.keyMessage] : []
        };
